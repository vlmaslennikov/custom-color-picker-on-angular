...
export class PickerDirective {
  ...
  @HostListener('keydown.arrowleft', ['$event.target']) arrowLeft(
    ...
    if (eventTarget.cellIndex === 0) {
      eventTarget.parentElement.cells[eventTarget.parentElement.cells.length-1].classList.add('selected');
      eventTarget.parentElement.cells[eventTarget.parentElement.cells.length-1].focus();
    } else {
  ...
  @HostListener('click', ['$event.target']) addColorByCursor(event: EventTarget):void {
    let eventTarget = event as any;
  ...
  @HostListener('keydown.enter', ['$event.target']) addColorByEnter(event: EventTarget):void {
    let eventTarget = event as any;
    this.setColor.emit(eventTarget.style.backgroundColor);
  }
